...
import { createProductApprovalChallenge } from "@/services/create-challenge/create-challenge-funding.server";
...
function safeRouteError(error: unknown) {
  ...
  return NextResponse.json({ error: { message: "Approval request failed." } }, { status: 400 });
}
...
export async function POST(request: Request) {
  try {
    ...
    return NextResponse.json({
      approval: await createProductApprovalChallenge(body.userToken, draftId, { ccnAccountId: context.ccnAccountId }),
    });
  } catch (error) {
