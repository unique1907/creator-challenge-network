...
import { verifyAndPublishChallenge } from "@/services/create-challenge/create-challenge-funding.server";
import { assertCreateChallengeDraftOwner, PublicSlugReservationError } from "@/services/create-challenge/create-challenge-store.server";
...
function safeRouteError(error: unknown) {
  ...
  return NextResponse.json({ error: { message: "Publish verification failed." } }, { status: 400 });
}
...
export async function POST(request: Request) {
  try {
    ...
    return NextResponse.json({
      publication: await verifyAndPublishChallenge(body.userToken, draftId, { ccnAccountId: context.ccnAccountId }),
    });
  } catch (error) {
