...
export default async function DashboardPage() {
    ...
    return (
      <main className="min-h-screen bg-[#050916] px-6 py-10 text-white">
        <div className="mx-auto max-w-2xl rounded-3xl border border-white/10 bg-[#0c1222] p-8">
          <p className="text-xs font-semibold uppercase tracking-[0.2em] text-cyan-200">Brand Workspace</p>
          <h1 className="mt-3 text-3xl font-semibold">Access not available</h1>
  ...
  return <BrandDashboard user={{ displayName: context.displayName, email: context.email, creatorAccess: context.creatorAccess, avatarImageUrl }} walletChip={walletChip} viewModel={viewModel} />;
}
...
async function getAvatarImageUrl(ccnAccountId: string) {
