...
includes(header, 'href: "/challenges", label: "Live Challenges"', "Header Live Challenges link must open the public challenge listing");
...
includes(landing, 'href: "/challenges", label: "Explore Live Challenges"', "Landing public CTA must route to the public challenge listing");
...
includes(signUpEntry, "Create your account", "Sign-up must render the role-first account title");
includes(signUpEntry, "Primary role", "Sign-up must ask for the required primary role");
...
includes(signUpEntry, "roleIntent={selected?.id ?? null}", "Continue must be blocked by AuthActions until role selection");
includes(signUpEntry, "Selected workspace", "User must receive visible selected-role confirmation");
...
includes(authActions, "Create account", "Sign-up auth method must support email/password account creation");
