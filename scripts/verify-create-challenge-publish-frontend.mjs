...
assert.ok(wizard.includes("async function publish("), "publish handler must exist");
...
assert.ok(wizard.includes("if (pending) return;") && wizard.includes("if (!ready)"), "click wrapper must guard disabled states explicitly");
...
assert.ok(wizard.includes("publishStepHeaderStatus(draft, status, currentLaunchReadiness)"), "publish step header must derive from canonical draft publication and readiness state");
